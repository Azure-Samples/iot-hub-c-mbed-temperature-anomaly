...
/* Thread used to update the lED, LCD and SPKR status */
static int LED_Update_Thread(void* threadArgument)
{
    ...
    while (1)
    {
		...
		/* Update temperature value from senor */
...
/* Callback for an Alarm Anomaly notification */
...
/* Callback for an Alarm Thresold notification */
...
/* Function to "link" IoTHub to the serialization library*/
...
/* Function used to get time from NTP server. This is required for the IoT Hub library to generate valid SAS Tokens for the secure connection */
...
int main(void)
{
    ...
    /* Create LED/LCD/SPKR status management thread */
    ...
    else
    {
        ...
        {
			/* This option is necessary only for the HTTP connection as the AMQP connection is maintained in the protocol itself */
            ...
            /* Create the model instance for the device meta data */
            ...
            {
            	/* Attach callback for receiving notificaions from IoT Hub */
                IOTHUB_CLIENT_RESULT setMessageResult = IoTHubClient_SetMessageCallback(iotHubClientHandle, IoTHubMessage, frdmDevice);
                ...
                else
                {
                    ...
                    frdmDevice->ObjectType = "DeviceInfo";
					...
					/* Prepare metadata that will be sent in each telemetry nessage */
                    frdmDevice->ObjectName = (ascii_char_ptr)deviceId;
                    frdmDevice->ObjectType = "SensorEvent";
                    frdmDevice->Version = "1.0";
